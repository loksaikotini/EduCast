...
export default function TeacherDashboard() {
  ...
  const fetchOwnedClassrooms = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || 'Failed to fetch owned classrooms');
      }
  ...
  const handleCreatePersistentClassroom = async ({ name, subject }) => {
    ...
    try {
      const res = await fetchWithAuth('/classroom/create', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-600 to-pink-500 text-white p-4 md:p-8">
      <CreateClassroomModal
        ...
        onCreate={handleCreatePersistentClassroom}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-1 space-y-6">
          <section className="bg-white/20 backdrop-blur-md p-6 rounded-xl shadow-lg">
            ...
            <button
              ...
              className="w-full bg-yellow-500 hover:bg-yellow-600 text-gray-800 font-bold py-3 rounded-md transition duration-150"
            >
              Start New Video Class
            </button>
            {createdClassCode && (
              <p className="mt-3 text-sm text-green-200 bg-green-700/50 p-2 rounded">
                Live class started! Code: <span className="font-mono">{createdClassCode}</span> (Navigating...)
              </p>
            )}
          </section>
          ...
          <section className="bg-white/20 backdrop-blur-md p-6 rounded-xl shadow-lg">
            ...
            <button
              ...
              className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-md transition duration-150 flex items-center justify-center"
            >
        ...
        <div className="md:col-span-2 bg-white/10 backdrop-blur-md p-6 rounded-xl shadow-lg">
          ...
          {!loading && ownedClassrooms.length === 0 && (
            <p className="text-center text-pink-200 py-8">You haven't created any classrooms yet. <br/>Use the button on the left to create one!</p>
          )}
                ...
                <div className="flex items-center space-x-2">
                    ...
                    Manage <FiArrowRight className="ml-2"/>
                     ...
                     <button
                        ...
                        className="bg-red-500 hover:bg-red-600 text-white p-2 rounded-md transition duration-150"
