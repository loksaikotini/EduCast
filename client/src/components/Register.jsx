      ...
      <div className="max-w-md w-full bg-white rounded-lg shadow-xl p-8 space-y-6">
        ...
        <div className="flex justify-center space-x-6 border-b pb-4">
          <label className="flex items-center space-x-2 cursor-pointer">
            <input type="radio" name="role" value="student" className="form-radio h-5 w-5 text-indigo-600" checked={role === 'student'} onChange={() => setRole('student')} />
            <span className="text-gray-700">I am a Student</span>
          </label>
          <label className="flex items-center space-x-2 cursor-pointer">
            <input type="radio" name="role" value="teacher" className="form-radio h-5 w-5 text-indigo-600" checked={role === 'teacher'} onChange={() => setRole('teacher')} />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button type="submit" disabled={loading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-md font-semibold transition duration-150 ease-in-out disabled:opacity-50">
