...
export default function Classroom() {
  ...
  const handleCreateVideoClass = () => {
    ...
    navigate(`/meeting/${videoCode}`);
  };

  const handleJoinVideoClass = () => {
    if (!videoMeetingCode.trim()) return alert('Please enter a class code for the video meeting.');
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {error && !error.toLowerCase().includes("chat") && <p className="container mx-auto bg-red-100 text-red-700 p-3 rounded-md my-4 text-sm">{error}</p>}
      {error && error.toLowerCase().includes("chat") && <p className="container mx-auto bg-yellow-100 text-yellow-700 p-3 rounded-md my-4 text-sm">{error}</p>}
          ...
          <div className="flex-grow p-4 overflow-y-auto">
            ...
            <section className="bg-indigo-50 p-4 rounded-lg mb-4">
              <h3 className="text-lg font-semibold text-indigo-800 mb-3 flex items-center">
                <FiVideo className="mr-2" /> Live Video Class
              </h3>
              {isTeacher ? (
                <button
                  onClick={handleCreateVideoClass}
                  className="w-full bg-yellow-500 hover:bg-yellow-600 text-gray-800 font-bold py-2 px-4 rounded-md transition duration-150"
                >
                ...
                <div className="space-y-2">
                  <input
                    ...
                    className="w-full border border-gray-300 bg-white text-gray-800 p-2 rounded-md focus:ring-2 focus:ring-yellow-500"
                    value={videoMeetingCode}
                    onChange={e => setVideoMeetingCode(e.target.value.toUpperCase())}
                  />
                  <button
            ...
            {activeTab==='chat' && (
              <div className="flex flex-col h-full">
                <div className="flex-1 overflow-y-auto border p-3 rounded-md mb-3 bg-gray-50 space-y-3">
                  {messages.length === 0 && <p className="text-sm text-gray-400 text-center">No messages. Start chatting!</p>}
                  {messages.map((msg,idx)=>(<div key={msg._id||idx} className={`flex ${msg.sender?._id===user?.id?'justify-end':'justify-start'}`}><div className={`max-w-[75%] p-2 rounded-lg shadow ${msg.sender?._id===user?.id?'bg-indigo-500 text-white':'bg-gray-200 text-gray-800'}`}><p className="text-xs font-semibold mb-0.5">{msg.sender?.name||msg.senderName||'User'}</p><p className="text-sm break-words">{msg.text}</p><p className="text-xs opacity-70 mt-1 text-right">{new Date(msg.timestamp).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</p></div></div>))}
                  <div ref={chatEndRef}/>
                </div>
                <form onSubmit={handleSendMessage} className="flex sticky bottom-0 bg-white py-2">
                  <input type="text" className="flex-1 border border-gray-300 rounded-l-md px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Message..." value={inputMsg} onChange={e=>setInputMsg(e.target.value)}/>
                  <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 sm:px-4 py-2 rounded-r-md font-semibold flex items-center"><FiSend size={16} className="mr-0 sm:mr-2"/><span className="hidden sm:inline">Send</span></button>
                </form>
              </div>
            )}
          </div>
